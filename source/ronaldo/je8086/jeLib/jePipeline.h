...
namespace jeLib
{
	...
	public:
		 ...
		 * and within 1..3; {1,2,3} is the four-stage split. `_cores` is optional
		 * CPU affinity, one entry per stage including the parent; empty pins
		 * nothing. Check valid() before use. */
		JePipeline(Je8086& _je, const std::vector<int>& _bounds, const std::vector<int>& _cores);
		~JePipeline();
	...
	private:
		...
		void stageMain(int _stage, int _core);
